export function createSpotlightCard(business) {
  ...
  card.innerHTML = `
  <div class="upper"><h3>${business.name}</h3></div>
  <div class="lower">
    <img src="${business.image}" alt="${business.name} Logo" width="100" height="100" loading="lazy">
    <div class="info"> 
      <p><b>Phone:</b> ${business.phone}</p>
      <p><b>Membership Level:</b> ${
        business.membershipLevel === 3
          ? "Gold"
          : business.membershipLevel === 2
          ? "Silver"
          : "N/A"
      }</p>
      <a href="${business.website}" target="_blank" rel="noopener noreferrer"><b>URL:</b> ${business.website}</a>
    </div>
  </div>`;
